...
const create_user = async () => {
    ...
    try {
        ...
        await $fetch('https://tt88.ru/backendapi/users/register', {
            method: 'POST',
            headers: {
...
const fetchUsers = async (page = 1) => {
    try {
        const token = authStore.token;
        const response = await $fetch<{ users: any[], totalPages: number, currentPage: number }>('https://tt88.ru/backendapi/users/users', {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`,
            },
