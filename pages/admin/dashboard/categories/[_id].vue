...
const createSubcategory = async () => {
    ...
    try {
        const token = authStore.token;
        const categoryId = route.params._id;
        await $fetch(`http://localhost:5000/api/subcategories/${categoryId}/subcategories`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${token}`,
            },
...
const deleteSubcategory = async (subcategoryId: string) => {
    try {
        const token = authStore.token;
        const categoryId = route.params._id;
        await $fetch(`http://localhost:5000/api/subcategories/${categoryId}/subcategories/${subcategoryId}`, {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${token}`,
            },
