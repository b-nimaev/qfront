...
const fetchSubcategories = async (page = 1) => {
    try {
        ...
        const response = await $fetch<{ subcategories: any[], categoryName: string | null, totalPages: number, currentPage: number }>(`https://tt88.ru/backendapi/categories/${categoryId}/subcategories`, {
            method: 'GET',
            headers: {
...
const createSubcategory = async () => {
    ...
    try {
        const token = authStore.token;
        const categoryId = route.params._id;
        await $fetch(`https://tt88.ru/backendapi/categories/${categoryId}/subcategories`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${token}`,
            },
...
const deleteSubcategory = async (subcategoryId: string) => {
    try {
        const token = authStore.token;
        const categoryId = route.params._id;
        await $fetch(`https://tt88.ru/backendapi/categories/${categoryId}/subcategories/${subcategoryId}`, {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${token}`,
            },
